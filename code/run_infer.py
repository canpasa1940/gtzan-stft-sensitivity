...
X=np.load(d+"/X_test.npy", mmap_mode='r'); y=np.load(d+"/y_test.npy")
...
for sd in seeds:
    ...
    np.save(o,P)
    print("%d/%d seed%s  segacc=%.4f  %.1fs"%(nf,hp,sd,(P.argmax(1)==y).mean(),time.time()-t))
np.save(OUT+"/y_test.npy", y)
